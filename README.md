...
К сожалению, добавление собственных методов к встроенным типам в Python требует или применения субклассов, или использования велосипедов типа forbiddenfruit (уже немножко умер) / fishhook (еще сыроват). А ведь в C# эта возможность встроена из коробки, аргхх...
...
def _supermemo2(repetition: dict, user_result: float) -> dict:
    ...
    if user_result >= DataOperations.level_good:  # Correct response
        if repetition["repetition_number"] == 0:  # + 1 day
            ...
            repetition["time_to_repeat"] = (datetime.now() + timedelta(days=6 * repetition["easiness_factor"])).strftime(datetime_format)
        repetition["repetition_number"] += 1
    else:  # Incorrect response
        repetition["repetition_number"] = 0

    repetition["easiness_factor"] = repetition["easiness_factor"] + (0.1 - (5 - 5 * user_result) * (0.08 + (5 - 5 * user_result) * 0.02))
